...
echo "./dbperf -db llrb $ARGS $LOAD"
./dbperf -db llrb $ARGS $LOAD
...
echo "./dbperf -db llrb $ARGS $LOAD $UPSERTS $DELETES $READS"
./dbperf -db llrb $ARGS $LOAD $UPSERTS $DELETES $READS
go tool pprof -svg dbperf dbperf.pprof  > pprof.svg
go tool pprof -alloc_space -svg dbperf dbperf.mprof  > alloc_space.svg
echo
